# -*- coding: utf-8 -*-
"""custom_encoder_v4.py

Changes from v3:
- Stage A now trains on LJSpeech + LibriSpeech train-clean-100 (combined ~124h, 252 speakers)
- ModalityAdapter: added dropout (0.15 in Q-Former, 0.1 on query tokens) to reduce overfitting
- Stage A config: MAX_STEPS_A=15000, WARMUP_STEPS_A=500, LR floor raised from 0.05 to 0.10
- Early stopping patience raised from 2 to 4
- Stage A training starts fresh (not resumed) due to multi-speaker distribution shift
...
GRAD_ACCUM_A        = 4
LR_A                = 2e-4
WARMUP_STEPS_A      = 500      # increased from 300
MAX_STEPS_A         = 15000
SAVE_EVERY_A        = 1500
LOG_EVERY_A         = 50
MAX_TEXT_LEN        = 128
...
COMBINED_TRAIN_CSV  = 'train_combined.csv'
...
# ── Download LJSpeech if not present ───────────────────────────────────────
if not Path(DATA_ROOT).exists():
    print('Downloading LJSpeech...')
    os.system('wget -q https://data.keithito.com/data/speech/LJSpeech-1.1.tar.bz2')
    os.system('tar -xjf LJSpeech-1.1.tar.bz2')
    print('LJSpeech done.')
else:
    print('LJSpeech already present.')

# ── Download LibriSpeech train-clean-100 if not present ────────────────────
if USE_LIBRISPEECH and not Path(f'{LIBRISPEECH_ROOT}/train-clean-100').exists():
    print('Downloading LibriSpeech train-clean-100 (~6.3 GB)...')
    os.system('wget -q https://www.openslr.org/resources/12/train-clean-100.tar.gz')
    os.system('tar -xzf train-clean-100.tar.gz')
    print('LibriSpeech done.')
elif USE_LIBRISPEECH:
    print('LibriSpeech already present.')

# ── Build LJSpeech manifest ─────────────────────────────────────────────────
def build_lj_manifest():
    ...
    return df[['wav_path', 'text', 'speaker']]

# ── Build LibriSpeech manifest ──────────────────────────────────────────────
def build_librispeech_manifest(root):
    records = []
    libri_root = Path(root) / 'train-clean-100'
        ...
        with open(trans_file) as f:
            for line in f:
                ...
                if len(parts) != 2:
                    continue
                utt_id, text = parts
                wav_path = trans_file.parent / f'{utt_id}.flac'
                if wav_path.exists():
                    ...
                    if 5 <= len(text) <= 190:
                        records.append({
                            ...
                            'text'    : text,
                            'speaker' : f'LS_{speaker_id}',
                        })
    return pd.DataFrame(records)

# ── Build or load combined manifest ────────────────────────────────────────
if not Path(COMBINED_TRAIN_CSV).exists():
    print('Building combined manifest...')
    lj_df = build_lj_manifest()
    print(f'  LJSpeech: {len(lj_df)} utterances')

    if USE_LIBRISPEECH:
        ls_df = build_librispeech_manifest(LIBRISPEECH_ROOT)
        print(f'  LibriSpeech: {len(ls_df)} utterances')
        combined = pd.concat([lj_df, ls_df], ignore_index=True)
    else:
        combined = lj_df

    # Use LJSpeech val split from existing val.csv if present,
    # otherwise split combined 90/10
    if Path(VAL_CSV).exists():
        lj_val = pd.read_csv(VAL_CSV)
        val_paths = set(lj_val['wav_path'].tolist())
        val_df   = combined[combined['wav_path'].isin(val_paths)]
        train_df = combined[~combined['wav_path'].isin(val_paths)]
    else:
        train_df, val_df = train_test_split(combined, test_size=0.10, random_state=42)

    train_df.to_csv(COMBINED_TRAIN_CSV, index=False)
    val_df.to_csv(COMBINED_VAL_CSV, index=False)
    print(f'Combined train: {len(train_df)} | val: {len(val_df)}')
    speakers = combined['speaker'].nunique()
    print(f'Total speakers: {speakers}')
else:
    train_df = pd.read_csv(COMBINED_TRAIN_CSV)
    val_df   = pd.read_csv(COMBINED_VAL_CSV)
    print(f'Loaded combined splits: train={len(train_df)} | val={len(val_df)}')
    print(f'Speakers: {train_df["speaker"].nunique()}')
...
class ModalityAdapter(nn.Module):
    ...
    def forward(self, encoder_output: torch.Tensor) -> torch.Tensor:
        ...
        queries = self.query_dropout(self.query_tokens.expand(B, -1, -1))  # (B, 32, 768) + dropout
...
train_ds = StageADataset(COMBINED_TRAIN_CSV if USE_LIBRISPEECH else TRAIN_CSV)
val_ds   = StageADataset(COMBINED_VAL_CSV   if USE_LIBRISPEECH else VAL_CSV)
...
def get_lr(step):
    ...
    return max(0.10, 0.5 * (1.0 + math.cos(math.pi * progress)))  # floor raised 0.05 -> 0.10
...
print(f'Starting fresh — no resume (multi-speaker distribution shift requires clean start)')
...
step            = 0       # fresh start — multi-speaker requires clean training
accum_step      = 0
best_val        = float('inf')
running_loss    = 0.0
...
while step < MAX_STEPS_A: # Changed MAX_STEPS to MAX_STEPS_A
    for batch in train_loader:
        ...
        with torch.amp.autocast('cuda', enabled=FP16):
            loss = forward_pass(mel, ids, mask) / GRAD_ACCUM_A
        ...
        if accum_step == GRAD_ACCUM_A: # Changed GRAD_ACCUM to GRAD_ACCUM_A
            ...
            if step % SAVE_EVERY_A == 0: # Changed SAVE_EVERY to SAVE_EVERY_A
                ...
                if val_loss < best_val:
                    best_val = val_loss
                    no_improve_count = 0
                    torch.save(ckpt, f'{OUTPUT_DIR_A}/best.pt')
                    print(f'  ↳ New best: {best_val:.4f}')
                else:
                    no_improve_count += 1
                    print(f'  No improvement ({no_improve_count}/{EARLY_STOP_PATIENCE})')
                    if no_improve_count >= EARLY_STOP_PATIENCE:
                        print(f'Early stopping after {EARLY_STOP_PATIENCE} validations without improvement')
                        # restore best weights
                        best_ckpt = torch.load(f'{OUTPUT_DIR_A}/best.pt', map_location=device)
                        adapter.load_state_dict(best_ckpt['adapter_state'])
                        llm.load_state_dict(best_ckpt['lora_state'])
                        print(f'Restored best weights (step {best_ckpt["step"]}, val_loss={best_ckpt["val_loss"]:.4f})')
                        writer.close()
